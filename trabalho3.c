...
int defineMatrizChegagem(int largura, int altura, Imagem1C* mask, int **mat) {
    ...
    return existe_buraco;
}

int segregaImagem(int *flag_master, int *symbol, int **matriz_checagem, int altura, int largura) {
    while (*flag_master)
    {
        *flag_master = 0;
        for (int i = 0; i < altura; i++)
        ...
        if (*flag_master == 0)
        {
            for (int i = 0; i < altura && symbol_atualizado == 0; i++)
            {
                for (int j = 0; j < largura && symbol_atualizado == 0; j++)
                {
                    if (matriz_checagem[i][j] == BRANCO)
                    {
                        *symbol += 1; // atualiza o symbol
                        matriz_checagem[i][j] = *symbol;
                        *flag_master = 1;
                        symbol_atualizado = 1;
                    }
...
Rect* encontraFuros(Imagem1C* mask, int* n_furos)
{
    ...
    *n_furos = 0;
    ...
    int existe_buraco = defineMatrizChegagem(largura, altura, mask, matriz_checagem);
    ...
    segregaImagem(&flag_master, &symbol, matriz_checagem, altura, largura);
